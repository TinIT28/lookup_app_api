...
export class PostService {
  ...
  async create(userId: string, post: CreatePostDto) {
    try {
      ...
      newPost.user = userId;
      if (newPost.images) {
        newPost.images = await this.cloudinaryService.convertImagesCloudinary(
          newPost.images,
        );
      }
  ...
  async update(postId: string, post: UpdatePostDto, userId: string) {
    try {
      ...
      if (post.images && foundPost.images !== post.images) {
        foundPost.images = await this.cloudinaryService.convertImagesCloudinary(
          post.images,
        );
      }
  ...
  async delete(postId: string, userId: string) {
    try {
      ...
      const deleteComments = this.commentService.deleteCommentByIds(
        foundPost.comments,
      );
